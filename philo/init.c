...
int	init_data(t_data *data, t_philo *philos, pthread_mutex_t *forks, char **argv)
{
	int	num_philos;
	...
	data->dead_flag = 0;
	return (init_forks(forks, num_philos) && init_philos(philos, data, forks, argv));
}
...
int	init_philos(t_philo *philos, t_data *data, pthread_mutex_t *forks, char **argv)
{
	int	i;
	int	num_philos;
	...
	while (i < num_philos)
	{
		...
		philos[i].must_eat_count = (argv[5]) ? ft_atol(argv[5]) : -1;
